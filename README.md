...
一款极简的原生 Android 照片拼接工具。在应用内相册中多选照片、调整顺序，并以纵向或横向合并后直接保存到系统相册。

## 功能

- 首页直接展示设备相册，点击即多选，最多 30 张（超出会有提示）
- 选中照片后底部出现 Merge 按钮，进入拼接页
- 纵向 / 横向无缝拼接预览（Coil 异步加载，滚动流畅）
- 长按缩略图后左右拖动连续排序
- 高质量 JPEG 导出到 `Pictures/iMerge`
- 导出完成后明确提示保存位置，可选择保留或删除拼接前原图
- 删除本地原图时使用 Android 系统确认；云端或不支持删除的来源会安全保留
- Release 版本每次启动自动检查 GitHub Releases 更新，展示更新说明并征求同意
- 同意后由系统仅在 Wi-Fi 下后台下载，校验 SHA-256 后自动打开 Android 安装界面
- 大图按 2400 万像素与 16000 像素边长预算自动缩放
- 支持浅色和深色系统主题，中英文界面，无广告、无水印

## 权限

- 照片读取权限（Android 13+ 为 `READ_MEDIA_IMAGES`，旧版本为 `READ_EXTERNAL_STORAGE`），用于在应用内展示和拼接相册照片；支持 Android 14 的「仅授权部分照片」模式
- 保存拼接图通过 `MediaStore` 完成，不需要写入权限
